/*
 * $Header: /CVSRoot/CoreOS/Services/ntp/ntp/libntp/mfp_mul.c,v 1.1.1.1 1998/10/30 22:18:15 wsanchez Exp $
 *
...
#define LOW_MASK  ((1<<(FRACTION_PREC/2))-1)
#define HIGH_MASK (LOW_MASK << (FRACTION_PREC/2))

void
mfp_mul(o_i, o_f, a_i, a_f, b_i, b_f)
     int32 *o_i;
     u_int32 *o_f;
     int32 a_i;
     u_int32 a_f;
     ...
     u_int32 b_f;
{
  int32 i, j;
  u_int32  f;
  u_short a[4];			/* operand a */
  u_short b[4];			/* operand b */
  u_long  c[4];			/* result c */
  ...
  for (i = 0; i < 4; i++)	/* we do assume 32 * 32 = 64 bit multiplication */
	...
	if (((c[(i+j)/2] >> 1) + (result_low >> 1)) & (1<<(FRACTION_PREC - 1)))
	  result_high++;	/* propagate overflows */

	c[(i+j)/2]   += result_low; /* add up partial products */

	if (((c[(i+j+1)/2] >> 1) + (result_high >> 1)) & (1<<(FRACTION_PREC - 1)))
...
#ifdef DEBUG
  if (debug > 6)
    printf("mfp_mul: 0x%04x%04x%04x%04x * 0x%04x%04x%04x%04x = 0x%08lx%08lx%08lx%08lx\n",
