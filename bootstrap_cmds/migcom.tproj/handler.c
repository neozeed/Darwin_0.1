 ...
 *
 *
 *	$Header: /CVSRoot/CoreOS/Commands/NeXT/bootstrap_cmds/migcom.tproj/handler.c,v 1.1.1.1.16.2 1999/03/16 16:11:59 wsanchez Exp $
 *
 * HISTORY
    ...
    statement_t *stats;
{
	...
	switch (stat->stKind)
	{
	  ...
	  case skUImport:
	    break;
	  default:
	    fatal("WriteHandler(): bad statement_kind_t (%d)",
		  (int) stat->stKind);
	}
    WriteEpilog(file, stats);
}
