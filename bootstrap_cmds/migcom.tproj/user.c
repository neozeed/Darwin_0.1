 ...
 *	the User module
 *
 *  $Header: /CVSRoot/CoreOS/Commands/NeXT/bootstrap_cmds/migcom.tproj/user.c,v 1.1.1.1.8.2 1999/03/16 16:12:02 wsanchez Exp $
 *
 * HISTORY
    ...
    routine_t *rt;
{
	...
	fprintf(file, "\t/* Maximum request size %d */\n",
		rt->rtMaxRequestSize);
#endif	NeXT
    ...
    routine_t *rt;
{
    ...
    {
#if	NeXT
	fprintf(file, "\tmsg_result = msg_send(&InP->Head, %s, %s);\n",
	    rt->rtSendTime != argNULL ? "SEND_TIMEOUT" : "MSG_OPTION_NONE",
	    rt->rtSendTime != argNULL ? rt->rtSendTime->argVarName : "0");
#else	NeXT
...
#if	NeXT
	fprintf(file, "\treturn msg_send(&InP->Head, %s, %s);\n",
	    rt->rtSendTime != argNULL ? "SEND_TIMEOUT" : "MSG_OPTION_NONE",
	    rt->rtSendTime != argNULL ? rt->rtSendTime->argVarName : "0");
#else	NeXT
...
{
#if	NeXT
    const char *msg_option;

    if (rt->rtWaitTime != argNULL && rt->rtSendTime != argNULL)
	msg_option = "SEND_TIMEOUT|RCV_TIMEOUT";
    else if (rt->rtWaitTime != argNULL)
	msg_option = "RCV_TIMEOUT";
    else if (rt->rtSendTime != argNULL)
	msg_option = "SEND_TIMEOUT";
    else
	msg_option = "MSG_OPTION_NONE";

    fprintf(file,"\tmsg_result = msg_rpc(&InP->Head, %s, %s, %s, %s);\n",
	    msg_option, "sizeof(Reply)",
	    rt->rtSendTime != argNULL? rt->rtSendTime->argVarName : "0",
	    rt->rtWaitTime != argNULL? rt->rtWaitTime->argVarName : "0");
#else	NeXT
    ...
    register argument_t *arg;
{
    ...
    fprintf(file, "\tmsg_size_delta = (%d * %s%s + 3) & ~3;\n",
    ...
    routine_t *rt;
{
	...
	fprintf(file, "\tif (((msg_size < %d)",
		rt->rtReplySize);
#if	NeXT
	fprintf(file, " || (msg_size > %d)", rt->rtMaxReplySize);
#endif	NeXT
    } else
	fprintf(file, "\tif (((msg_size != %d)",
		rt->rtReplySize);
...
#if	NeXT
    if (btype->itTypeSize % 4 != 0)
	fprintf(file, "\tmsg_size_delta = (%d * OutP->%s + 3) & ~3;\n",
    ...
    routine_t *rt;
{
    ...
    fprintf(file, "mig_external %s %s", ReturnTypeStr(rt), rt->rtUserName);
#if	NeXT
    fprintf(file, " (\n");
