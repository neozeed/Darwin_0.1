...
#ifndef lint
static char  rcsid[] = "@(#)$Id: conf.c,v 1.7 1998/10/30 18:44:55 wsanchez Exp $";
#endif /* !lint */
...
getcfname()
{
...
#if NETINFO
	{
		...
		if (cflocation != NULL)
                  {
                    if (access(cflocation,F_OK))
                      {
                        usrerr("WARNING: Cannot open %s (specified by NetInfo): %s\n"
                               "WARNING: Using default cf file: %s\n",
                               cflocation, errstring(errno),
                               _PATH_SENDMAILCF);
