	...
	if (event->kind == mouse_click)
	  {
...
#ifdef HAVE_NS
            if (FRAME_NS_P (f) &&
                FRAME_EXTERNAL_MENU_BAR (f) && XINT (event->y) == -1)
              {
		...
		item = ns_map_event_to_object (event, f);

		XSETFASTINT (event->y, 1);
...
#ifdef HAVE_NS /* Formerly we treated an up with no down as a click event.  */
	    /** 
	      We'll do this on NS, cause we don't get separate up down
	      events, just that the bar moved 
	      **/
	    if (!CONSP (start_pos))
