	...
	void *cancelparm, char *tag1, char *tag2, char *tag3 )
{
	char		*dn, *ftag = NULL;
	char		**dns = NULL;
	int		max, i, err, scope = LDAP_SCOPE_BASE, phase, tries;
	LDAPFiltInfo	*fi;
	LDAPMessage	*tmpcand;
	...
	*res = NULL;
	...
	tryagain:
		tries++;
		for ( fi = ldap_getfirstfilter( (LDAPFiltDesc *)ld->ld_filtd, ftag,
		    ufncomp[ncomp] ); fi != NULL;
		    fi = ldap_getnextfilter( (LDAPFiltDesc *)ld->ld_filtd ) )
		{
	...
	char *tag1, char *tag2, char *tag3 )
{
	...
	char	*pbuf;
	int	ncomp, pcomp, i, err = -1;
	char 	path[MAXPATHLEN];
		
	if ( ldap_locate_path( path, SUFFIX_FILTERFILE ) == NULL ) {
		return( ld->ld_errno = LDAP_LOCAL_ERROR );
	}
		
	/* initialize the getfilter stuff if it's not already */
	if ( ld->ld_filtd == NULL && ldap_ufn_setfilter( ld, path )
	    == NULL ) {
		return( ld->ld_errno = LDAP_LOCAL_ERROR );
	}
