...
- (LUArray *)init
{
	...
	if (rover != nil) [rover addObject:self];
...
- (void)release
{
	...
	[lock lock];
	retainCount--;	
	if ((retainCount == 1) && (rover != nil)) callOffTheDog = YES;
	else if (retainCount == 0) pleaseReleaseMe = YES;
	[lock unlock];

	if (callOffTheDog) [rover removeObject:self];
