...
handle_io(int ignored)
{
	...
	while ((nchars = read(console, (char *)&buf, sizeof(buf))) > 0)
	{
		...
		while (nchars--)
		{
			if ((*cp == 'c') || (*cp == 'C'))
			{
				sys_msg(debug, LOG_DEBUG, "Binding aborted");
				aborted = 1;
				alert_close();
				return;
			}
...
alert_open(const char *language)
{
	...
	{
		sys_msg(debug, LOG_ERR, "console open failed: %s", strerror(errno));
		aborted = 1;
		return;
	}
	...
	{
		sys_msg(debug, LOG_ERR, "console F_GETFL fcntl failed: %s",
			strerror(errno));
		aborted = 1;
		close(console);
		return;
	...
	{
		sys_msg(debug, LOG_ERR, "console F_SETFL fcntl failed: %s",
			strerror(errno));
		aborted = 1;
		close(console);
		return;
	...
	{
		sys_msg(debug, LOG_ERR, "console TIOCGETP ioctl failed: %s",
			strerror(errno));
		aborted = 1;
		close(console);
		return;
	...
	{
		sys_msg(debug, LOG_ERR, "console TIOCSETP ioctl failed: %s",
			strerror(errno));
		aborted = 1;
		close(console);
		return;
