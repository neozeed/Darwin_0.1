	   ...
	   )
{
	...
	socket_unlock();
	if (s < 0) {
		sys_msg(debug, LOG_ERR, "multi_call: socket: %m");
		return (RPC_FAILED);
	}

	i = 1;
	if (setsockopt(s, SOL_SOCKET, SO_BROADCAST, &i, sizeof(int)) < 0) {
		sys_msg(debug, LOG_ERR, "multi_call can't broadcast: %m");
	}
	...
	for (callno = 0; callno <= NRETRIES; callno++) {
		...
		case -1:
			sys_msg(debug, LOG_ERR, "select failure: %m");
			continue;
		case 0:
			sys_msg(debug, LOG_DEBUG, "multicall timeout: %u+%u",
