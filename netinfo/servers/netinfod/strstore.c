...
strstore(strtree *thetree, const char *string)
{
	...
	sys_msg(debug, LOG_DEBUG, "strstore ranstrcmp impossible result");
	return (NULL);
}
...
insertnode(strtree *thetree, strtree thenode)
{
	...
	if (res < 0) {
		...
		sys_msg(debug, LOG_DEBUG, "insertnode something already there");
	} else if (res > 0) {
...
strdelete(strtree *thetree, const char *string)
{
	...
	if (*thetree == NULL) {
		sys_msg(debug, LOG_DEBUG, "strdelete tree already deleted");
		return;
	}
	res = ranstrcmp(string, (*thetree)->string);
	if (res < 0) {
