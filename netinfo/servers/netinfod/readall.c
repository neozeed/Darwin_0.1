     ...
     )
{
	...
	if (ni_parent(ni, &object.nio_id, &object.nio_parent) != NI_OK) {
		socket_lock();
		sys_msg(debug, LOG_DEBUG, "couldn't get parent of %d\n",
	...
	if (ni_read(ni, &object.nio_id, &object.nio_props) != NI_OK) {
		socket_lock();
		sys_msg(debug, LOG_DEBUG, "couldn't read %d\n",
			object.nio_id.nii_object);
		return (FALSE);
	}
...
#endif
		sys_msg(debug, LOG_DEBUG, "couldn't get children of %d\n",
			object.nio_id.nii_object);
		return (FALSE);
	}
	socket_lock();
	...
	if (!xdr_bool(xdr, &true) ||
		...
		sys_msg(debug, LOG_ERR, "couldn't xdr %d: %m",
		       object.nio_id.nii_object);
#if !ENABLE_CACHE
		ni_proplist_free(&object.nio_props);
#endif
...
proxy_term(void)
{
    sys_msg(debug, LOG_WARNING, "readall proxy terminating due to SIGTERM");
    /*
