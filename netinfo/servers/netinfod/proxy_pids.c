...
void add_proxy(const int pid, const unsigned long addr)
{
	...
	{
		sys_msg(debug, LOG_ALERT, "Proxy table overflow (%u proxies)",
			nproxies);
		exit(1);
	}
...
void remove_proxy(const int pid)
{
	...
	for (i = 0; i < nproxies; i++)
		if (readall_proxy_pids[nproxies][0] == pid) break;


	for (i++; i < nproxies; i++)
	{
		readall_proxy_pids[i][0] = readall_proxy_pids[i-1][0];
		readall_proxy_pids[i][1] = readall_proxy_pids[i-1][1];
	}
...
unsigned long get_proxy(const int pid)
{
	...
	for (i = 0; i < nproxies; i++)
	{
		if (readall_proxy_pids[nproxies][0] == pid)
			return readall_proxy_pids[nproxies][1];
	}
...
bool_t kill_proxies(void)
{
    ...
    if (killpg(process_group, SIGTERM) != 0)
	{
		switch(errno)
		{
			case ESRCH:
			...
			default:
				sys_msg(debug, LOG_WARNING, "couldn't terminate proxies - %m");
			return(FALSE);
		}
	}
	...
	if (killpg(process_group, SIGKILL) != 0)
	{
		...
		switch(errno)
		{
			case ESRCH:
				...
				return(TRUE);
			default:
				sys_msg(debug, LOG_WARNING, "couldn't kill proxies - %m");
				return(FALSE);
		}
	}
    ...
    if (killpg(getpid(), SIGKILL) != 0)
	{
		switch(errno)
		{
			case ESRCH:
				...
				return(TRUE);
			default:
				sys_msg(debug, LOG_WARNING, "couldn't test-kill proxies - %m");
				return(FALSE);
		}
	}
