	...
	)
{
	...
	unsigned long masteraddr, myaddr;
	ni_status ret;

	if (!getmaster(db_ni, &master, &mastertag)) {
		/* no master! */
		sys_msg(debug, LOG_ERR, "tag %s: can't determine master's host name", mytag);
		return (void)NULL;
	}
	...
	if (masteraddr == 0) {
		...
		sys_msg(debug, LOG_ERR, "tag %s: can't determine master's address", mytag);
		return (void)NULL;
	}

	myaddr = sys_address();

	ret = checkserves(masteraddr, mastertag, myaddr, mytag);
		return (void)NULL;
}
...
ni_status checkserves(unsigned long masteraddr, ni_name mastertag, unsigned long myaddr, ni_name mytag)
{
	...
	if (sock < 0) {
		sys_msg(debug, LOG_WARNING, "sanitycheck can't connect to "
		       "%s/%s - %m",
		       inet_ntoa(mastersin.sin_addr), mastertag);
		return NI_FAILED;
	}
