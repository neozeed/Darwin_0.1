	...
	file:(char *)fname
{
	...
	while (NULL != (line = [self getLineFromFile:fp category:cat]))
	{
		...
		item = [parser parse:line category:cat];
...
- (LUArray *)allItemsWithCategory:(LUCategory)cat file:(char *)fname
{
	...
	[vstamp setBanner:"FFAgent validation stamp"];
...
- (LUDictionary *)userWithName:(char *)name
{
	return [self itemWithKey:"name" value:name 
		category:LUCategoryUser file:"passwd"];
}
...
- (LUArray *)allUsers
{
	return [self allItemsWithCategory:LUCategoryUser file:"passwd"];
}
...
- (LUArray *)allGroupsWithUser:(char *)name
{
	...
	sprintf(fpath, "%s/passwd", etcDir);
	if (stat(fpath, &st) < 0) ts = 0;
	else ts = st.st_mtime;

	vstamp = [[LUDictionary alloc] init];
	[vstamp setBanner:"FFAgent validation stamp"];
	[allWithUser addValidationStamp:
		[self stamp:vstamp file:fpath time:ts]];
	[vstamp release];
