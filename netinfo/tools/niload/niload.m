...
ni_status destroy_children(void *domain, ni_id *dir)
{
	ni_status status;
	int i;
	ni_idlist children;
...
void raw_dir(void *domain, ni_id *dir, NSMutableDictionary *item, BOOL isTop)
{
	int i, len;
	NSArray *children;
	...
	children = [[item objectForKey:@"CHILDREN"] retain];
	[item removeObjectForKey:@"CHILDREN"];
	...
	{
		fprintf(stderr,
			"%s: Can't %s directory: %s\n",
			MYNAME, (isTop ? "write" : "create"), ni_error(status));
		exit(1);
	}
	...
	if (isTop)
	{
		...
		{
			fprintf(stderr,
				"%s: Can't destroy child directories: %s\n",
	...
	for (i = 0; i < len; i++)
	{
		NSDictionary *immutableChild = [children objectAtIndex:i];
		child = [immutableChild mutableCopy];
		if (isTop) raw_dir(domain, dir, child, NO);
		else raw_dir(domain, &child_dir, child, NO);
		[child release];
	}
...
void raw_load(void *domain, char *path, char *file)
{
	char *rawbytes, *p;
	char buf[256];
	size_t len;
	...
	{
		fprintf(stderr,
			"%s: raw load requires an absolute directory pathname, found %s\n",
			 MYNAME, path);
		exit(1);
	}
	...
	if (file == NULL)
	{
		...
		while (len > 0)
		{
			...
			len = read(0, buf, 256);
			if (len > 0) rawbytes = realloc(rawbytes, i + len + 1);
		}

		rawbytes[i] = '\0';

		pList = [[[NSString stringWithCString:rawbytes]
			propertyList] retain];
		free(rawbytes);
	}
	else
	{
		pList = [[NSDictionary alloc] initWithContentsOfFile:
	...
	{
		fprintf(stderr,
			"%s: bad raw format: top level directory is not an NSDictionary\n",
			 MYNAME);
		[pool release];
		exit(1);
	}

	top = [pList mutableCopy];
	[pList release];

	raw_dir(domain, &nidir, top, YES);
...
main(int argc, char *argv[])
{
	...
	else if (!strcmp(format, "exports"))
	{
		cat = LUCategoryExport;
		strcpy(nidirname, "/exports");
	}
	...
	else if (!strcmp(format, "mountmaps"))
	{
		if (mntmap_arg == 0)
		{
			fprintf(stderr,
				"%s: a mountmap name is required for loading mountmap format\n", MYNAME);
			usage();
		}
		cat = LUCategoryMountmap;
		sprintf(nidirname, "/mountmaps/%s", argv[mntmap_arg]);
	}
	else
	{
