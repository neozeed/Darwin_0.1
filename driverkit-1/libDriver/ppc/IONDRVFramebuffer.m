...
+ (BOOL)probe:device
{
    ...
    if( NDRVForDevice( device)) {
        ...
        else if( 0 == strncmp("IMS,tt3d", name, strlen("IMS,tt3d")))
...
// ATI patches. getStartupMode doesn't.
...
enum {
	kPantherOff_Regs		= 0x06000000L,
        kPantherOff_SSTATUS		= 0x000000ECL,
};

void regwNoswap( volatile UInt8 * io, UInt32 regindex, UInt32 regdata) {

    *((UInt32 *)(io + regindex)) = (regdata);
    __asm__ volatile("eieio");
}

void
ix3dInterruptHandler( void * interrupt, void * istate, IOIX3DNDRV * vars )
{
    regwNoswap(vars->panther, kPantherOff_SSTATUS, 0x30 );

    IOEnableInterrupt( interrupt);
}
...
{
    *handler = (IOInterruptHandler) ix3dInterruptHandler;
    *ipl = 30;
    ...
    setDisplayMode:(IOFBDisplayModeID)modeID depth:(IOFBIndex)depthIndex page:(IOFBIndex)pageIndex
{
    ...
    err = [super setDisplayMode:modeID depth:depthIndex page:pageIndex];
...
- (IOReturn) open
{
    ...
    if( err == noErr) {
        ...
        panther = (volatile UInt8 *)(((UInt32)config.mappedFramebuffer) & 0xffff0000)
			+ kPantherOff_Regs;
	[self enableInterrupt:0];
    }
    return( err);
