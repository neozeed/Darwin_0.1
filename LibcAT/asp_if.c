 ...
 *
 * $Id: asp_if.c,v 1.19.6.3 1999/03/16 15:49:19 wsanchez Exp $
 */
...
#define LOG_ERR(name)     {if (logfp) { \
	fprintf(logfp, "%s(%d): err=%d,errno=%d %s", name, getpid(), *SPError, cthread_errno(), newline); \
	fflush(logfp); }}
#else
...
/*To fix bug 2210045*/
		...
		if (writev(SessRefNum, iov, 2) == -1) {
			*SPError = (cthread_errno() == EINVAL) ? ASPERR_ParamErr : ASPERR_SessClosed;
			at_rwl_readunlock(&(sp->RWLock));
			LOG_ERR(rn);
			return -1;
		}
...
/*
 * Name: SPCommand (client only)
 */
int
SPCommand(int SessRefNum,
	char *CmdBlock,
	int CmdBlockSize,
	char *ReplyBuffer,
	int ReplyBufferSize,
	...
	int NoWait)
{
	...
	/*
	 * send the command request to peer
	 */
	if (SendCommand(SessRefNum,
			CmdBlock, CmdBlockSize, SPError, ASPFUNC_Command) == -1)
		return -1;
	 ...
	 * get the response
	 */
	return SPGetReply(SessRefNum, ReplyBuffer,
...
l_cmdDone:
	...
	*(unsigned short *)addr.net = DefSessionTimer;
...
/*
 * Name: SPGetStatus (client only)
 */
int
SPGetStatus(at_inet_t *SLSEntityIdentifier,
	char *StatusBuffer,
	...
	char *SLSName)
{
  ...
  if (SLSFamily == PF_INET) {
	unsigned short port;
	asp_fspkt_t fspktbuf, *fspkt = &fspktbuf;
	int len, len_left, sum = 0;
	struct sockaddr_in in_addr;

	if (((port = GetListenPort(1, (void *)SLSEntityIdentifier, SLSName)) == 0)
			|| ((descr = socket(PF_INET, SOCK_STREAM, 0)) == -1)) {
		goto l_atalk;
	}
	...
	in_addr.sin_addr.s_addr = *(unsigned long *)SLSEntityIdentifier;
	if (connect(descr, (void *)&in_addr, sizeof(in_addr)) == -1) {
		close(descr);
		goto l_atalk;
	}
	...
	fspkt->TotalLen = 0;
	fspkt->Reserved = 0;
	if (send(descr, (char *)fspkt, sizeof(*fspkt),MSG_EOR) != sizeof(*fspkt)) {
		*SPError = ASPERR_SessClosed;
		goto l_error;
	}
	if (recv(descr, (char *)fspkt, sizeof(*fspkt), 0) != sizeof(*fspkt)) {
		*SPError = ASPERR_SessClosed;
		goto l_error;
	}
	len_left = fspkt->TotalLen;
	if (len_left > StatusBufferSize) {
		SET_ERRNO(ERANGE);
		*SPError = ASPERR_BufTooSmall;
		goto l_error;
	}
	while (len_left && ((len = recv(descr,
			(char *)&StatusBuffer[sum], len_left, 0)) > 0)) {
		sum += len;
		len_left -= len;
	}
	if (len_left) {
...
l_atalk:
	...
	if (at_send_to_dev(descr, ASPIOC_ClientBind, 0, 0) == -1) {
		*SPError = ASPERR_NoMoreSessions;
		goto l_error;
	}

	status_cmd.SLSEntityIdentifier = *SLSEntityIdentifier;
	status_cmd.Retry = CmdRetry;
	status_cmd.StatusBufferSize = StatusBufferSize;
	ic_len = sizeof(status_cmd);
	if ((rval = at_send_to_dev(descr, ASPIOC_GetStatus,
			(char *) &status_cmd, &ic_len)) == -1) {
		*SPError = ASPERR_SystemErr;
		goto l_error;
	}
	if (rval > StatusBufferSize) {
		SET_ERRNO(ERANGE);
		*SPError = ASPERR_BufTooSmall;
		goto l_error;
	}
	if (read(descr, StatusBuffer, rval) == -1) {
		*SPError = ASPERR_SystemErr;
		goto l_error;
	}
  }

	*ActRcvdStatusLen = rval;
	close(descr);
...
/*
 * Name: SPOpenSession (client only)
 */
int
SPOpenSession(at_inet_t *SLSEntityIdentifier,
	void (*AttnRoutine)(),
	int *SessRefNum,
	OSErr *SPError,
	int SLSFamily,
	char *SLSName)
{
  ...
  if (SLSFamily == PF_INET) {
	...
	if (fspkt->ErrorCode) {
		*SPError = fspkt->ErrorCode;
		goto l_error;
	}

	len_left = fspkt->TotalLen;
	while (len_left && ((len = recv(descr,
			&optbuf[fspkt->TotalLen-len_left],sizeof(optbuf),0)) > 0))
		len_left -= len;
	opt = (asp_opt_t *)optbuf;
	if (!(sp = createSess(descr, SPError))) {
		goto l_error;
	}
	sp->SessType = ASPDT_TCP_FS;
	sp->UserType = 1;
	sp->Quantum = (opt->tag == ASPFS_DataQuantumTag) ?
		*(int *)opt->val : ASPDEF_QuantumSize;
	at_rwl_writeunlock(&(sp->RWLock));
  } else {
l_atalk:
	descr = ATsocket(ATPROTO_ASP);
	...
	if (at_send_to_dev(descr, ASPIOC_ClientBind,
			(char *) SLSEntityIdentifier, &ic_len) == -1) {
		*SPError = ASPERR_NoMoreSessions;
		goto l_error;
	}

	open_cmd.SLSEntityIdentifier = *SLSEntityIdentifier;
	open_cmd.Retry = CmdRetry;
	open_cmd.TickleInterval = DefTickleInterval;
	open_cmd.SessionTimer = DefSessionTimer;
	ic_len = sizeof(open_cmd);
	if ((rval = at_send_to_dev(descr, ASPIOC_OpenSession,
			(char *) &open_cmd, &ic_len)) != 0) {
		*SPError = rval;
		goto l_error;
	}
...
/*
 * Name: SPWrite (client only)
 */
int
SPWrite(int SessRefNum,
	char *CmdBlock,
	int CmdBlockSize,
	char *WriteData,
	int WriteDataSize,
	...
	int NoWait)
{
	...
	if (sp->SessType == ASPDT_TCP_FS) {
		...
		char fspktbuf[sizeof(asp_fspkt_t)+ASPDEF_MaxCmdSize];
		asp_fspkt_t *fspkt = (asp_fspkt_t *)fspktbuf;
		...
		fspkt->Flags = 0;
		fspkt->Command = ASPFUNC_Write;
		fspkt->RequestID = FSRequestID++;
		fspkt->ErrorCode = CmdBlockSize;
		fspkt->TotalLen = CmdBlockSize + WriteDataSize;
		fspkt->Reserved = 0;
		memcpy((char *)(fspkt+1), CmdBlock, CmdBlockSize);
		fspktlen += CmdBlockSize;
		if ( (send(SessRefNum, (char *)fspkt, fspktlen,
				WriteDataSize ? 0 : MSG_EOR) != fspktlen) ||
			(WriteDataSize && (send(SessRefNum, WriteData,
				WriteDataSize,MSG_EOR) != WriteDataSize)) ) {
			*SPError = (cthread_errno() == EINVAL) ? ASPERR_ParamErr : ASPERR_SessClosed;
			at_rwl_readunlock(&(sp->RWLock));
			LOG_ERR(rn);
			return -1;
		...
		/*
		 * return if caller doesn't want to wait for the reply
		 */
		if (NoWait) {
			*SPError = ASPERR_NoError;
			at_rwl_readunlock(&(sp->RWLock));
		...
		at_rwl_readunlock(&(sp->RWLock));
		/*
		 * send the write request to peer
		 */
		if (SendCommand(SessRefNum, CmdBlock,
				CmdBlockSize, SPError, ASPFUNC_Write) == -1)
			return -1;

		/*
		 * return if caller doesn't want to wait for the write continue
		 */
		if (NoWait) {
			*SPError = ASPERR_NoError;
			return 0;
		...
		/*
		 * wait for the write continue
		 */
		if ((rval = SPGetRequest(SessRefNum, ReplyBuffer,
				ReplyBufferSize, &ReqRefNum,
				&ReqType, ActRcvdReplyLen, SPError))) {
			return rval;
		}
		 ...
		 * write the data
		 */
		BuffSize = *(unsigned short *)ReplyBuffer;
		if (WriteDataSize > BuffSize)
			WriteDataSize = BuffSize;
		if (SPCmdReply(SessRefNum, ReqRefNum,
				0, WriteData, WriteDataSize, SPError))
			return -1;
	}

	/*
	 * get the response for the write request
	 */
	if ((rval = SPGetReply(SessRefNum, ReplyBuffer, ReplyBufferSize, 
			       CmdResult, ActRcvdReplyLen, SPError)))
		return rval;

	*ActLenWritten = WriteDataSize;
	*SPError = ASPERR_NoError;
	return 0;
}
	...
	OSErr SPError)
{
	...
	catd = catopen(MF_LIBAT, NL_CAT_LOCALE);
#endif
...
/*
 * Name: SPEnableSelect (client & server, supplemental call)
 */
int
SPEnableSelect(int SessRefNum,
	OSErr *SPError)
{
	session_t *sp;
#ifdef ASPDEBUG
	char *rn = "SPEnableSelect";
#endif
	...
	if (sp->SessType == ASPDT_ATP_AS) {
		if (at_send_to_dev(SessRefNum, ASPIOC_EnableSelect, 0, 0) == -1) {
			if (cthread_errno() == EINVAL)
				*SPError = ASPERR_ParamErr;
			else
				*SPError = ASPERR_SystemErr;
...
/*
 * Name: XPGetRefNum (server only, supplemental call)
 */
int
XPGetRefNum(int SLSRefNum,
	int *RefNum,
	int *RefNumCnt,
	OSErr *SPError)
{
...
/*
 * Name: XPGetSession (server only, hack for a third-party SW vendor)
 */
int
XPGetSession(int SLSRefNum,
	int *SessRefNum,
	OSErr *SPError)
{
	static int descr = -1;
