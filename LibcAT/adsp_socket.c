 ...
 *
 * $Id: adsp_socket.c,v 1.7.12.3 1999/03/16 15:49:19 wsanchez Exp $
 */
...
#include <netat/h/ADSP.h>
#include <netat/h/adsp_frames.h>
#include <netat/h/adsp_ioctl.h>
...
at_sndcmd(int fd, struct adspcmd *cmd, int _cmd)
{
	...
	while (((rc = at_send_to_dev(fd,
		_cmd, cmd, &len)) < 0) && (cthread_errno() == EINTR))
		;
	return rc;
}
