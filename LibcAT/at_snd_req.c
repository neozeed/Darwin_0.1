 ...
 *
 * $Id: at_snd_req.c,v 1.10.24.3 1999/03/16 15:49:20 wsanchez Exp $
 */
	...
	at_atpreq	*treq_buff;
{
	...
	NET_NET(ddp->dst_net, treq_buff->at_atpreq_to_net);
	...
	while ((status = ATPsndreq(fd, buff, len, 1)) < 0) {
		if (cthread_errno() == EINTR || cthread_errno() == EAGAIN)
			continue;
		perror("ATPsndreq");
		return -1;
	}
	...
	int		nowait;
{
	...
	if (retry && (retry->interval <= 0)) {
		...
		/* validate dest addr */
	if (_validate_at_addr(dest) == -1)
		return (-1);

	treq.at_atpreq_type			= ATP_CMD_TREQ;
        UAL_ASSIGN(treq.at_atpreq_treq_user_bytes, userdata);
        UAS_UAS(treq.at_atpreq_to_net, dest->net);
        treq.at_atpreq_to_node			= dest->node;	
        treq.at_atpreq_to_socket		= dest->socket;
